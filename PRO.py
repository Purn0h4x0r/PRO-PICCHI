...
try:
    rq = requests.get('https://raw.githubusercontent.com/Purn0h4x0r/PRO-PICCHI/main/Approval.txt').text
...
- 😽 𝙍𝙚𝙢𝙚𝙢𝙗𝙚𝙧 𝙏𝙝𝙞𝙨 𝙉𝙖𝙢𝙚 - Md Purno Shah✨
\033[1;97m--------------------------------------------------
"""
...
a=requests.get("https://raw.githubusercontent.com/Purn0h4x0r/PRO-PICCHI/main/Approval.txt").text
b=str(a)
key1=open("/storage/emulated/0/android8.txt",'r').read()
key2=str(key1)  
if key2 in b:
...
def result():
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            okl = os.listdir("PICCHI-OK");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN OK FILE %s]'%(H,P,H));print('%s '%(O))
            ...
            try:
                ppp = _azimvau_dapunta_("PICCHI-OK/%s"%(files)).read().splitlines()
        ...
        try:
            cpl = os.listdir("PICCHI-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_azimvau_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _azimvau_dapunta_("PICCHI-CP/%s"%(files)).read().splitlines()
...
def cek_result():
    ...
    for memek in buka_baju:
        ...
        _format_ = "\r %s[%sPICCHI-CP%s] %s          "%(K,P,K,kontol)
